...
class _UserViewState extends State<UserView> {
  ...
  late BehaviorSubject<List<UserDataMModal>> userStreamController;
  Stream<List<UserDataMModal>> get users => userStreamController.stream;

  @override
  void initState() {
    super.initState();
    userStreamController = new BehaviorSubject<List<UserDataMModal>>();
    getUserData();
  }

  getUserData() async{
    List<UserDataMModal> userData= await GoogleSheetsController.getAllUsers();
    userStreamController.sink.add(userData);
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          StreamBuilder(
            stream: users,
            builder: (context, AsyncSnapshot<List<UserDataMModal>> snapshot){
                ...
                return Expanded(
                  child: ListView(
                    children: List.generate(snapshot.data!.length, (index) => GestureDetector(
                      onTap: (){
                        Navigator.of(context).push(MaterialPageRoute(builder: (context) => EditUserInfoView(user: snapshot.data![index],))).then((value){
                          getUserData();
                        });
                      },child: EmployeeCard(snapshot.data![index]))),
                  ),
                );
              }else{
                return Column(
        ...
        children: <Widget>[
          ...
          new Row(
            children: <Widget>[
              ...
              Text(" ${employee.mobileNumber}",style: subHeaderTextStyle,)
            ],
          ),
        ],
      ),
